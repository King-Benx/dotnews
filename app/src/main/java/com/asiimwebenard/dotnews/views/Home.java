...
public class Home extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        titleHeader.setText(Objects.requireNonNull(this.getActivity()).getResources().getString(R.string.local_news));
        ...
        swipeRefreshLayout.setOnRefreshListener(() -> newsViewModal.getNewsReportObservable().observe(getActivity(), newsReport ->
        {
    ...
    private void observeViewModel(NewsViewModal newsViewModal) {
        if(this.getActivity() != null) {
            newsViewModal.getNewsReportObservable().observe(this.getActivity(), newsReport -> {
                if (newsReport != null) {
                    newsAdapter = new NewsAdapter(newsReport.getArticles(), new RecyclerViewListener() {
                        @Override
                        public void onItemClick(Article article) {
                            Navigation.findNavController(Objects.requireNonNull(getView())).navigate(HomeDirections.actionHomeToDetail(article));
                        }
